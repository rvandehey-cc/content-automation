...
export default function EditSiteProfilePage() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      // Scraper
      if (config.scraper) {
        setContentSelectors(config.scraper.contentSelectors?.join(', ') || '')
        setWaitTime(String(config.scraper.waitTime || '2000'))
        setTimeout(String(config.scraper.timeout || '60000'))
        setMaxRetries(String(config.scraper.maxRetries || '2'))
      }
      
      // Blog post
      if (config.blogPost) {
        setBlogContentSelector(config.blogPost.contentSelector || '')
        setBlogDateSelector(config.blogPost.dateSelector || '')
        setBlogTitleSelector(config.blogPost.titleSelector || '')
        setBlogExcludeSelectors(config.blogPost.excludeSelectors?.join('\n') || '')
      }
      
      // Page
      if (config.page) {
        setPageContentSelector(config.page.contentSelector || '')
        setPageExcludeSelectors(config.page.excludeSelectors?.join('\n') || '')
      }
      
      // Processor
      if (config.processor) {
        setCustomRemoveSelectors(config.processor.customRemoveSelectors?.join('\n') || '')
        setPreserveBootstrapClasses(config.processor.preserveBootstrapClasses !== false)
        setRemoveAllClasses(config.processor.removeAllClasses === true)
        setRemoveAllIds(config.processor.removeAllIds !== false)
      }
      
      // Images
      if (config.images) {
        setImagesEnabled(config.images.enabled !== false)
        setMaxConcurrent(String(config.images.maxConcurrent || '5'))
        setImageTimeout(String(config.images.timeout || '30000'))
        setAllowedFormats(config.images.allowedFormats?.join(',') || 'jpg,jpeg,png,gif,webp')
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Build config object (same as new page)
      ...
      router.push(`/site-profiles/${profileId}`)
    } catch (err) {
  ...
  // Reuse the same form structure as new page
  return (
    <div className="container mx-auto py-10 max-w-5xl">
      <div className="mb-6">
        <h1 className="text-3xl font-bold">Edit Site Profile</h1>
        <p className="text-muted-foreground mt-1">
          Update scraping and processing settings for this site
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* All the same form fields as new page - I'll import the form component or duplicate */}
        {/* For now, I'll create a shared component, but let's duplicate for simplicity */}
        {/* Basic Information */}
        <Card>
          <CardHeader>
        ...
        <div className="flex gap-4">
          <Button type="submit" variant="light" disabled={saving} aria-label="Save changes to site profile">
